...
class Dsv7ValidatorVrlTest < Minitest::Test
  ...
  def test_minimal_vrl_is_valid
    result = validate_string(vrl_minimal)
  ...
  def test_missing_required_elements_are_reported
    ...
    missing.each do |el|
      assert_includes result.errors, "Vereinsergebnisliste: missing required element '#{el}'"
    end
  end

  def test_wettkampf_wk_art_allows_a_and_n
    content = vrl_minimal.sub('WETTKAMPF:1;A;', 'WETTKAMPF:1;N;')
    r = validate_string(content)
    assert r.valid?, r.errors.inspect

    bad = vrl_minimal.sub('WETTKAMPF:1;A;', 'WETTKAMPF:1;Q;')
