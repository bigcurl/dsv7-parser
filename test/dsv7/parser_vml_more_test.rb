...
class Dsv7ParserVmlMoreTest < Minitest::Test
  def vml_sample
    <<~DSV
  ...
  def test_accepts_bom_prefix
    bom = "\xEF\xBB\xBF"
    events = Dsv7::Parser.parse_vereinsmeldeliste(bom + vml_sample).to_a
    assert_equal :format, events.first[0]
    assert_equal 'Vereinsmeldeliste', events.first[1][:list_type]
  end

  def test_reads_from_path
    path = 'tmp/parser_vml_more_input.DSV7'
    begin
      File.write(path, vml_sample)
      enum = Dsv7::Parser.parse_vereinsmeldeliste(path)
