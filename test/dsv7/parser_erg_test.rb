...
class Dsv7ParserErgTest < Minitest::Test
  def collect_events
    events = []
    Dsv7::Parser.parse_wettkampfergebnisliste(sample) do |type, payload, line_number|
  ...
  def test_streams_format_event
    ...
    assert_equal 'Wettkampfergebnisliste', fmt[1][:list_type]
  ...
  def test_returns_enumerator_without_block
    enum = Dsv7::Parser.parse_wettkampfergebnisliste(sample)
